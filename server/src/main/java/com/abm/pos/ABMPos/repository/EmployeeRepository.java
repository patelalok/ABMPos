...
public interface EmployeeRepository extends JpaRepository<EmployeeDao,Integer> {
    ...
    @Query(value = "SELECT distinct e.username,\n" +
            "SUM(l.l.sale_quantity) quantity,\n" +
            "SUM(l.cost * l.l.sale_quantity) cost,\n" +
            "SUM(l.retail * l.l.sale_quantity) retail,\n" +
            "SUM((l.retail * l.l.sale_quantity) - (l.cost * l.l.sale_quantity)) profit\n" +
            "from transaction t\n" +
